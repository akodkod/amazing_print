...
RSpec.describe 'AmazingPrint' do
  ...
  describe 'Hash' do
    ...
    it 'colored with colors option set to :all' do
      expect(@hash.ai(colors: :all)).to eq <<~EOS.strip
        {
    ...
    it 'colored single line' do
      ...
      expect(@hash.ai(multiline: false, hash_format: :rocket)).to eq(
        "{ 1\e[0;37m => \e[0m{ :sym\e[0;37m => \e[0m{ \"str\"\e[0;37m => \e[0m{ [ 1, 2, 3 ]\e[0;37m => \e[0m{ { :k => :v }\e[0;37m => \e[0m\e[1;33mHash < Object\e[0m } } } } }"
      )
    end
  end
  ...
  describe 'Hash with several keys' do
    ...
    it 'does not align delimiters when align_delimiter is false for symbol hash format' do
      test_hash = { [0, 0, 255] => :yellow, :bloodiest_red => 'rgb(255, 0, 0)', 'magenta' => 'rgb(255, 0, 255)' }
      out = test_hash.ai(colors: :none, indent: -2, hash_format: :symbol, sort_keys: true, align_delimiter: false)
      expect(out).to eq <<~EOS.strip
        {
          [ 0, 0, 255 ] => :yellow,
